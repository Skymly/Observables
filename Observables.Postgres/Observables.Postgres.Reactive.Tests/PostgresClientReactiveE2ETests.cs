...
public sealed class PostgresClientReactiveE2ETests(PostgresTestServerFixture fixture)
{
    ...
    public async Task Listen_receives_Notify_from_another_session()
    {
        ...
        var hub = PostgresService.For<IE2EHubReactive>(listener);

        using var timeout = CancellationTokenSource.CreateLinkedTokenSource(cancellation);
        timeout.CancelAfter(DefaultTimeout);
        var receive = hub.Ping.Timeout(DefaultTimeout).FirstAsync().ToTask(timeout.Token);
    ...
    public async Task Notify_from_proxy_is_observed_by_second_Listen_connection()
    {
        ...
        var hub = PostgresService.For<IE2EHubReactive>(publisher);
        await hub.PublishPing(payload).Timeout(DefaultTimeout).FirstAsync().ToTask(timeout.Token);
