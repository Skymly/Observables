...
namespace Observables.RestAPI
{
    ...
    partial class RequestBuilderImplementation : IRequestBuilder
    {
        ...
        readonly RestApiSettings settings;
        ...
        )
        {
            ...
            settings = RestApiSettings ?? new RestApiSettings();
                ...
                return interfaceGenericHttpMethods.GetOrAdd(
                    ...
                    _ =>
                        new RestMethodInfoInternal(
                            ...
                            restMethodInfo.RestApiSettings
                        )
                );
            }
        ...
        )
        {
            ...
            // IObservable<T> — task core path; SystemReactive generator wraps with adapter
            if (restMethod.ReturnType.GetTypeInfo().IsGenericType
                && restMethod.ReturnType.GetGenericTypeDefinition() == typeof(IObservable<>))
            {
                var taskFuncMi = typeof(RequestBuilderImplementation).GetMethod(
                    nameof(BuildTaskFuncForMethod),
                    BindingFlags.NonPublic | BindingFlags.Instance
            ...
            // R3 Observable<T>
            if (restMethod.ReturnType.GetTypeInfo().IsGenericType
                && restMethod.ReturnType.GetGenericTypeDefinition() == typeof(global::R3.Observable<>))
            {
                var r3FuncMi = typeof(RequestBuilderImplementation).GetMethod(
                    nameof(BuildR3ObservableFuncForMethod),
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                var r3Func = (MulticastDelegate?)
                    (
                        r3FuncMi!.MakeGenericMethod(
                            restMethod.ReturnResultType,
                            restMethod.DeserializedResultType
                        )
                    ).Invoke(this, [restMethod]);

                return (client, args) => r3Func!.DynamicInvoke(client, args);
            }
            ...
            throw new ArgumentException(
                $"Method \"{restMethod.MethodInfo.Name}\" is invalid. REST methods must return Task<T>, ValueTask<T>, or R3 Observable<T>."
            );
        }
...
#pragma warning restore CA2000
        ...
        Func<HttpClient, object[], global::R3.Observable<T?>> BuildR3ObservableFuncForMethod<T, TBody>(
            RestMethodInfoInternal restMethod
        )
        {
            ...
            return (client, paramList) =>
            {
                return global::R3.Observable.FromAsync(ct =>
                {
                    ...
                    return new ValueTask<T?>(taskFunc(client, cts.Token, paramList));
                });
            };
        }
