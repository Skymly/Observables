...
| # | 行动项 | 工作量 | 说明 |
|---|--------|--------|------|
| E3 | 诊断描述符补全 `description` + `helpLinkUri` | S | 全域 37 个 `DiagnosticDescriptor` 仅传 6 个基础参数，缺 `description` 和 `helpLinkUri`。用户在 IDE 错误列表悬停无法看详细说明和文档链接。补全后 `helpLinkUri` 指向 `Observables.Docs/docs/diagnostics.md#obsxxxx` |
| E4 | 7/8 域补 `// <auto-generated/>` 标记 | S | 仅 Events 域用 `GeneratedSourceHeader.ToSource()`；其余 7 域裸字符串拼接 `#pragma warning disable` + 命名空间。影响 IDE 格式化工具、覆盖率工具识别。统一用共享 `GeneratedSourceHeader` |
| E5 | 诊断文档补 Fix + 错误→正确代码示例 | M | `Observables.Docs/docs/diagnostics.md` 列出 37 个诊断的 ID/Severity/When，但缺 Fix 说明和代码示例。中英文同步补充 |

#### P3-C — 工程质量增强

| # | 行动项 | 工作量 | 说明 |
|---|--------|--------|------|
| E6 | 补未测试诊断用例（约 50%） | L | 已测试 ~19 个，未测试 ~25 个：Events OBS2003/2004（路由事件）、RestAPI OBS3001/3002/3003/3005、SignalR OBS4001/4002/4003/4006、Mqtt/WebSocket/Grpc/Sse/Nats 各 3-4 个。每诊断至少一个"触发 → 验证 DiagnosticResult"负面用例 |
| E7 | 测试基础设施去重（15 份 GeneratorTestHarness） | M | 每域独立实现 `GeneratorTestHarness.cs`（220-230 行），结构几乎相同。提取到 `Observables.TestSupport` 共享项目，泛型基类承载公共逻辑 |
...
| # | 行动项 | 工作量 | 说明 |
|---|--------|--------|------|
| E11 | 架构总览文档 + 贡献者指南 | M | `docs/design/` 缺 `architecture.md`（生成器管道、双后端切换机制）和 `contributor.md`（新增域步骤，面向人类贡献者，区别于面向 AI 的 AGENTS.md） |
| E12 | 6/8 域 Samples 补真实可运行示例 | M | Events/RestAPI 有真实示例；SignalR/Mqtt/Grpc/WebSocket/Sse/Nats 仅 `RegistrationDemo`（传 null 捕获异常）。参考仓内 E2E 测试的进程内服务器补真实示例 |
